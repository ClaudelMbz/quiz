...
const QuizSummary: React.FC<QuizSummaryProps> = ({ 
  ...
  if (percentage >= 80) {
    message = "Excellent job! You are an insulation expert.";
    colorClass = "text-green-600 dark:text-green-400";
  } else if (percentage >= 50) {
    message = "Not bad! A bit more effort to master the subject.";
    colorClass = "text-yellow-600 dark:text-yellow-400";
  } else {
    message = "Oops... You might need to review the insulation course.";
    ...
    <div className="max-w-4xl mx-auto w-full px-4 pb-12">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden mb-8 border border-transparent dark:border-slate-700 transition-colors duration-200">
        <div className="p-8 text-center bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Quiz Finished!</h2>
          <div className="my-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              {showReview ? "Hide Details" : "Review Mistakes"}
            </button>
            <button 
              onClick={onRestart}
      ...
      {showReview && (
        <div className="space-y-6 animate-fade-in-up">
          <h3 className="text-xl font-bold text-slate-800 dark:text-white ml-2">Detailed Review</h3>
